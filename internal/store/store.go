...
func Open(path string) (*Store, error) {
	...
	if err := migrateItemBudget(db); err != nil {
		db.Close()
...
func (s *Store) Add(ctx context.Context, in ItemInput) (Item, error) {
	...
	budget := budgetCents(in)
	res, err := s.db.ExecContext(ctx,
		`INSERT INTO items (name, qty, category, status, notes, budget_cents, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		in.Name, in.Qty, in.Category, StatusNeeded, in.Notes, budget, formatTime(created))
	if err != nil {
	...
	return Item{
		...
		Status: StatusNeeded, Notes: in.Notes, BudgetCents: budget, CreatedAt: created,
	}, nil
}
...
func migrateItemBudget(db *sql.DB) error {
